...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      await sendCloseNotification(thread, `Почтовый тред #${thread.thread_number} с ${thread.user_name} (${thread.user_id}) был запланированно закрыт ${thread.scheduled_close_name}`);
    }
  }
  ...
  async function scheduledCloseLoop() {
    ...
    setTimeout(scheduledCloseLoop, 60000);
  }
  ...
  commands.addGlobalCommand("close", "[opts...]", async (msg, args) => {
    ...
    await sendCloseNotification(thread, `Почтовый тред #${thread.thread_number} с ${thread.user_name} (${thread.user_id}) был закрыт ${closedBy}`);
  });
