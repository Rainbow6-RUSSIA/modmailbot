 ...
 formatStaffReplyEditNotificationThreadMessage(threadMessage) {
  ...
  formatLog(thread, threadMessages, opts = {}) {
    ...
    const lines = threadMessages.map(message => {
      ...
      let line = `[${moment.utc(message.created_at).utcOffset(3).format("DD.MM.YYYY HH:mm:ss")} МСК] `;
    ...
    const openedAt = moment.utc(thread.created_at).utcOffset(3).format("DD.MM.YYYY HH:mm:ss");
